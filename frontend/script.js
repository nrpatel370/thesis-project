...
function logout() {
  ...
  if (formulaEditor) {
    ...
    document.getElementById("toggleFormulaText").textContent = "Show";
  }
...
themeIcon.src =
  savedTheme === "dark" ? "./assets/darkmode.png" : "./assets/lightmode.png";
...
themeToggle.addEventListener("click", () => {
  ...
  themeIcon.src =
    newTheme === "dark" ? "./assets/darkmode.png" : "./assets/lightmode.png";
...
const FORMULA_FIELDS = [
  ...
  {
    id: "inputLabTotalPoints",
    key: "lab_total_points",
    label: "Lab total points",
  },
  {
    id: "inputAttendanceMult",
    key: "attendance_multiplier",
    label: "Attendance multiplier",
  },
  {
    id: "inputAttendanceTotalPoints",
    key: "attendance_total_points",
    label: "Attendance total points",
  },
  {
    id: "inputLabDenomFallback",
    key: "lab_denominator_fallback",
    label: "Lab max points (fallback)",
  },
  {
    id: "inputDdDenomFallback",
    key: "dd_denominator_fallback",
    label: "Debug Dungeon max points (fallback)",
  },
];
...
// Enter key triggers Register (the primary CRN action).
crnInput.addEventListener("keypress", (e) => {
  if (e.key === "Enter") registerBtn.click();
});

/** Return false and show an error message if the CRN format is invalid. */
function validateCrnInput(crn) {
  if (crn.length < 5 || crn.length > 6) {
...
/** Ask the backend whether this CRN already has any saved data. */
async function checkCrnExists(crn) {
  const response = await fetch(`${API_URL}/exists/${crn}`);
...
/**
 * Disable (or re-enable) all three CRN-section controls together.
 * Used to prevent double-clicks during the existence check and to lock
 * the controls once the user has entered the app.
 */
function setCrnControlsDisabled(disabled) {
  crnInput.disabled = disabled;
  registerBtn.disabled = disabled;
  loginBtn.disabled = disabled;
  document.getElementById("continueAsGuest").disabled = disabled;
}

/**
 * Shared activation path for both Register and Login.
 * Loads saved preferences + formula config, then opens the upload section.
 */
async function activateCrnMode(crn) {
  isGuestMode = false;
  currentUserId = crn;
  await Promise.all([loadSavedPreferences(), loadFormulaConfig()]);
  // Brief pause so the contextual message is readable before the page shifts.
  await new Promise((resolve) => setTimeout(resolve, 800));
  uploadSection.classList.add("active");
  setCrnControlsDisabled(true);
  showNavbarUser();
}
...
registerBtn.addEventListener("click", async () => {
  ...
  try {
    const exists = await checkCrnExists(crn);
    registerBtn.textContent = "Register";

    if (exists) {
      showMessage(
        crnMessage,
        "CRN already registered -- logging you in.",
        "info",
      );
    } else {
      showMessage(crnMessage, "CRN registered! You're all set.", "success");
    }

    await activateCrnMode(crn);
  } catch (err) {
    registerBtn.textContent = "Register";
    showMessage(crnMessage, "Something went wrong. Please try again.", "error");
    setCrnControlsDisabled(false);
  }
});
...
loginBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    if (exists) {
      showMessage(
        crnMessage,
        "Welcome back! Loading your saved preferences.",
        "success",
      );
    } else {
      showMessage(
        crnMessage,
        "No account found for this CRN -- registering now.",
        "info",
      );
    }
...
async function loadSavedPreferences() {
  ...
  try {
    ...
    if (prefs && typeof prefs === "object") {
      savedPreferenceState = {
        selected_columns: Array.isArray(prefs.selected_columns)
          ? prefs.selected_columns
          : [],
...
document
  ...
  .addEventListener("click", async () => {
    ...
    if (Object.keys(categories).length === 0) {
      showMessage(
        document.getElementById("categoryMessage"),
        "Please add at least one category with keywords",
        "error",
      );
      return;
    }
...
function renderCategorySelections(categories) {
  ...
  const attendanceCandidates = allColumns.filter((col) =>
    col.toLowerCase().includes("attendance"),
  );
  const finalExamCandidates = allColumns.filter((col) =>
    col.toLowerCase().includes("final exam part2"),
  );
  const attendanceDefault =
    attendanceCandidates.find(
      (col) => col.toLowerCase().trim() === "attendance total",
    ) ||
    attendanceCandidates[0] ||
    "";
  const finalExamDefault =
    finalExamCandidates.find((col) =>
      col.toLowerCase().includes("coding assessment"),
    ) ||
    finalExamCandidates[0] ||
    "";
  ...
  const CALCULATION_CATEGORIES = new Set([
    "labs",
    "debug_dungeon",
    "participation",
  ]);
  ...
  const renderCategoryGroup = (category, cols) => {
    ...
    const hasActiveCheckboxes =
      isCalc && cols.some((col) => !dropdownControlledCols.has(col));
    return `
  ...
  const categoriesHTML = Object.entries(categories)
    .map(([category, cols]) => renderCategoryGroup(category, cols))
    .join("");
  ...
  const setSelectionsCollapsed = (collapsed) => {
    ...
    collapsible.classList.toggle(
      "selections-collapsible--collapsed",
      collapsed,
    );
    toggleCollapseBtn.setAttribute(
      "aria-expanded",
  ...
  document
    .getElementById("attendanceColumnSelect")
    ?.addEventListener("change", updateCalculationScopeSummary);
  document
    .getElementById("finalExamColumnSelect")
    ?.addEventListener("change", updateCalculationScopeSummary);
...
function applySavedPreferencesToSelections() {
  ...
  const selectedColumnsSet = new Set(
    savedPreferenceState.selected_columns || [],
  );
  ...
  if (
    ...
    [...attendanceSelect.options].some(
      (opt) => opt.value === savedPreferenceState.selected_attendance_column,
    )
  ) {
  ...
  if (
    ...
    [...finalExamSelect.options].some(
      (opt) => opt.value === savedPreferenceState.selected_final_exam_column,
    )
  ) {
...
function setupCheckboxHandlers() {
  ...
  categoryCheckboxes.forEach((categoryBox) => {
    categoryBox.addEventListener("change", (e) => {
      ...
      const checked = e.target.checked;
      const itemsInCategory = document.querySelectorAll(
        `input[name="preference"][data-category="${category}"]`,
      );
      itemsInCategory.forEach((cb) => {
        if (!cb.disabled) cb.checked = checked;
      });
      updateMasterCheckbox();
    });
...
function updateCategoryCheckbox(category) {
  const categoryBox = document.querySelector(
    `.category-select[data-category="${category}"]`,
  );
...
function updateMasterCheckbox() {
  const masterCheckbox = document.getElementById("selectAllMaster");
  const allItems = [
    ...document.querySelectorAll('input[name="preference"]'),
  ].filter((cb) => !cb.disabled);
...
function getSelectedSingleColumns() {
  const selectedAttendance =
    document.getElementById("attendanceColumnSelect")?.value || "";
  const selectedFinalExam =
    document.getElementById("finalExamColumnSelect")?.value || "";
...
function updateCalculationScopeSummary() {
  ...
  const ddCount = selectedFields.filter(
    (f) => f.category === "debug_dungeon" || f.category === "participation",
  ).length;
...
async function persistUserPreferencesToServer(
  checkedColumns,
  selectedAttendance,
  selectedFinalExam,
) {
...
async function savePreferences(event) {
  ...
  try {
    const { selectedAttendance, selectedFinalExam } =
      getSelectedSingleColumns();
...
async function runDebugNormalization() {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok)
      throw new Error(data.error || "Debug normalization failed");
...
function displayDebugResults(debug) {
  ...
  const panel = document.createElement("div");
  panel.id = "debugPanel";
  const perStudentRows = (debug.per_student || [])
    .map(
      (row) => `
        <tr>
            <td>${row.student ?? ""}</td>
